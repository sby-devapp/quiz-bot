...
class ChatController(Controller):
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        print(f"[/start] User: {user.first_name} {user.last_name if user.last_name else ''} ({user.id})")
        await update.message.reply_text(
    ...
    async def random_question(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        question = self.chatService.get_random_question(chat_id=chat.id)
        await self.send_question(chat, question, context)
        

    async def stop(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if not await self._is_admin_owner(update, context):
            await update.message.reply_text("You must be an admin or owner to use this command.")
            return
        user, chat = self.get_user_chat(update, context)

        chat.settings()
        chat._settings.keep_receiving_questions = "no"
        chat._settings.save()

        await update.message.reply_text("You stopped the bot from sending random Questions")
